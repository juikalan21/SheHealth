...
def visualize_clusters(df, features_df, scaled_features, all_labels, n_best_clusters):
    plt.figure(figsize=(16, 16))  # Increase figure size
    gs = GridSpec(4, 2, wspace=0.4, hspace=0.6)  # Increase spacing between subplots
    ...
    for cluster in range(n_best_clusters):
        ax1.scatter(data_pca[all_labels == cluster, 0],
                    ...
                    label=f'Cluster {cluster}', s=80, alpha=0.7)  # Increase marker size
    ...
    sns.boxplot(x=all_labels, y=features_df['age'], ax=ax2, palette='viridis')  # Use a colorblind-friendly palette
    ...
    sns.boxplot(x=all_labels, y=features_df['bmi'], ax=ax3, palette='viridis')
    ax3.set_title('BMI Distribution by Cluster', fontsize=16)
    ax3.set_xlabel('Cluster', fontsize=14)
    ax3.set_ylabel('BMI', fontsize=14)
    ...
    x = np.arange(n_best_clusters)
    width = 0.25
    ax4.bar(x - width, cluster_chol, width, label='Total Cholesterol', color='#1f77b4')
    ax4.bar(x, cluster_hdl, width, label='HDL', color='#ff7f0e')
    ax4.bar(x + width, cluster_ldl, width, label='LDL', color='#2ca02c')
    ax4.set_title('Cholesterol Profiles by Cluster', fontsize=16)
    ax4.set_xlabel('Cluster', fontsize=14)
    ax4.set_ylabel('Level (mg/dL)', fontsize=14)
    ax4.set_xticks(x)
    ...
    ax5.bar(x - width / 2, cluster_systolic, width, label='Systolic', color='#1f77b4')
    ax5.bar(x + width / 2, cluster_diastolic, width, label='Diastolic', color='#ff7f0e')
    ax5.set_title('Blood Pressure by Cluster', fontsize=16)
    ax5.set_xlabel('Cluster', fontsize=14)
    ax5.set_ylabel('mmHg', fontsize=14)
    ax5.set_xticks(x)
    ax5.legend(fontsize=12)

    # 6. Correlations between features within each cluster
    for i, cluster in enumerate(range(min(n_best_clusters, 2))):
        ax = plt.subplot(gs[3, i])
        cluster_data = features_df[all_labels == cluster]
        corr = cluster_data.corr()
        mask = np.triu(np.ones_like(corr, dtype=bool))
        sns.heatmap(corr, mask=mask, cmap='coolwarm', vmin=-1, vmax=1,
                   annot=True, square=True, linewidths=.5, ax=ax, annot_kws={'fontsize': 12})  # Increase annotation font size
